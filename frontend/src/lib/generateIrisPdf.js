...
// ── Executive Brief pages (3 pages, CISO audience) ────────────────────────────

function execCover(exec, snap, date) {
  const m   = exec.key_metrics || {};
  const sev = m.severity || 'Unknown';
  const sc  = SEV_COLOR[sev] || '#94a3b8';
  const status = snap.current_status || exec.containment_status || 'Unknown';
  ...
  <div class="page" style="padding:0;display:flex;flex-direction:column;">
    <div style="background:${NAVY};padding:18mm 18mm 16mm;display:flex;flex-direction:column;min-height:158mm;">
      <div style="font-size:9px;font-weight:700;letter-spacing:0.15em;color:#4b6a9b;text-transform:uppercase;margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid #1e3a5f;">CONFIDENTIAL — EXECUTIVE BRIEF</div>
      <div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding:20px 0;">
        <div style="font-size:44px;font-weight:800;color:#fff;letter-spacing:-1.5px;line-height:1;margin-bottom:4px;">IRIS</div>
        <div style="font-size:11px;font-weight:400;color:#4b6a9b;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:6px;">Incident Response Intelligence Summary</div>
        <div style="font-size:10px;font-weight:600;color:#d97706;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:24px;">Executive Brief</div>
        <div style="font-size:20px;font-weight:700;color:#e2e8f0;line-height:1.4;margin-bottom:18px;max-width:480px;">${exec.headline || m.incident_name || 'Incident Brief'}</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          <span style="display:inline-block;padding:3px 10px;border-radius:3px;font-size:10px;font-weight:700;letter-spacing:0.06em;background:${sc};color:#fff;">${sev.toUpperCase()} SEVERITY</span>
          <span style="display:inline-block;padding:3px 10px;border-radius:3px;font-size:10px;font-weight:700;letter-spacing:0.06em;background:${stc};color:#fff;">${status.toUpperCase()}</span>
    ...
    <div style="flex:1;padding:14mm 18mm 12mm;display:flex;flex-direction:column;justify-content:space-between;">
      ...
      <div style="border-top:1px solid #e2e8f0;padding-top:10px;display:flex;justify-content:space-between;align-items:flex-end;">
        <div>
          <div style="font-size:10px;font-weight:600;color:#334155;margin-bottom:2px;">IRIS · Executive Brief · Powered by Claude AI</div>
          <div style="font-size:9px;color:#94a3b8;">AI-generated from analyst notes — human review required before external disclosure</div>
        </div>
        <div style="text-align:right;">
          <div style="font-size:9px;font-weight:700;letter-spacing:0.12em;color:#94a3b8;text-transform:uppercase;">Confidential</div>
...
function execSummaryPage(exec, date) {
  const m   = exec.key_metrics || {};
  const sev = m.severity || 'Unknown';
  const sc  = SEV_COLOR[sev] || '#94a3b8';
  const dwellAlert = m.dwell_time_hours > 72;
...
function execImpactRecsPage(impact, recs, date) {
  const rr  = impact.reputational_risk || 'Unknown';
  const rrc = SEV_COLOR[rr] || '#94a3b8';

  // Only Critical and High, max 6 cards total
  const topRecs = [
    ...(recs.immediate || []).filter(r => r.priority === 'Critical' || r.priority === 'High'),
    ...(recs.strategic || []).filter(r => r.priority === 'Critical' || r.priority === 'High'),
  ].slice(0, 6);

  const allRecs = [...(recs.immediate || []), ...(recs.strategic || [])];
  const lowerCount = allRecs.length - topRecs.length;

  return `
  <div class="page">
    ${pageHeader('Impact & Recommendations', 'Executive Brief — Page 3 of 3', date, 3, 3)}
    ...
    <div style="padding:10px 14px;border-left:3px solid #92400e;margin-bottom:12px;">
      <div style="font-size:10px;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:6px;">Business Impact</div>
      <div style="font-size:12px;line-height:1.8;color:#1e293b;">${impact.business_impact_narrative || ''}</div>
    </div>
    ...
    ${topRecs.length === 0 ? '<p style="font-size:11px;color:#94a3b8;font-style:italic;">No Critical or High priority actions identified.</p>' : `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
      ${topRecs.map(r => {
        const pc = PRI_COLOR[r.priority] || '#64748b';
        return `
          <div style="border-left:3px solid ${pc};padding:8px 10px 8px 12px;background:#fff;border:1px solid ${pc}22;border-left-width:3px;border-radius:4px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;">
              ${badge(r.priority, pc)}
              <span style="font-size:9px;color:#64748b;font-weight:600;">${r.owner}</span>
            </div>
            <div style="font-size:10px;font-weight:700;color:#0f172a;margin-bottom:4px;line-height:1.4;">${r.finding}</div>
            <div style="font-size:10px;color:#334155;line-height:1.5;margin-bottom:5px;">${r.action}</div>
            <div style="font-size:9px;color:#64748b;padding-top:5px;border-top:1px solid ${pc}20;">Due: ${r.deadline}</div>
          </div>`;
      }).join('')}
    </div>`}

    ${lowerCount > 0 ? `
    <div style="margin-top:10px;padding:8px 12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:4px;font-size:11px;color:#64748b;">
    ...
    <div style="margin-top:16px;padding:10px 0;border-top:1px solid #e2e8f0;">
      <div style="font-size:9px;color:#94a3b8;line-height:1.6;">
        <strong style="color:#64748b;">About this brief:</strong> Generated by IRIS from analyst-provided incident notes using Claude AI.
    ...
    ${pageFooter('Impact & Recommendations', date, 3, 3)}
  </div>`;
}
...
<body>
  ${execCover(exec, snap, date)}
  ${execSummaryPage(exec, date)}
  ${execImpactRecsPage(impact, recs, date)}
  <script>
    window.addEventListener('load', () => setTimeout(() => window.print(), 800));
  </script>
</body>
